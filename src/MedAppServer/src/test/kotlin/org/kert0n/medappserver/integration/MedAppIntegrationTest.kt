...
class MedAppIntegrationTest {
    ...
    fun `shared medkit workflow - multiple users`() {
        ...
        val user2 = userService.registerNewUser(UUID.randomUUID(), "password2", "127.0.0.1")
        val shareKey = medKitService.generateMedKitShareKey(medKit, user1.id)
        medKitService.addUserToMedKit(shareKey, user2.id)
    ...
    fun `insufficient quantity prevents treatment plan creation`() {
        val user = userService.registerNewUser(UUID.randomUUID(), "password", "127.0.0.1")
        val medKit = medKitService.createNew(user.id)
