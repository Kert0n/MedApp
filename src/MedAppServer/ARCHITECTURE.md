...
- **OpenAPI** (`OpenApiConfiguration`) — описывает публичный REST контракт.
...
| **VidalDrug** | Каталог препаратов | `name`, `formType`, `manufacturer`, `category`, `description` |

## Ключевые потоки

1. **Регистрация**: запрос на `/auth/register` с `secret` → создание `User` + возврат логина и ключа.
2. **Аутентификация**: HTTP Basic → `/auth/login` → JWT токен.
3. **Общая аптечка**: генерация ключа → присоединение по ключу → синхронизация содержимого.
4. **План лечения**: планируемый объем приема резервирует количество препарата и обновляется после фактического приема.

## Безопасность и приватность

- Сервер не хранит персональные данные, только технические идентификаторы.
